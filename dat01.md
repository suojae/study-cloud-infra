...
## 4. 정리
- **Host PC**는 **Wi-Fi 공유기**와 같은 역할을 하며, 공인 IP 주소를 통해 외부 네트워크와 통신한다.
- **가상 머신(VM)** 은 내부 네트워크에서 **내부 IP 주소**를 사용하여 통신하며, NAT를 통해 외부와의 연결이 가능해진다.
- **NAT**는 가상 머신의 내부 IP 주소를 **Host PC의 공인 IP 주소**로 변환하고, 응답을 다시 가상 머신으로 전달하는 **중계자** 역할을 한다.
